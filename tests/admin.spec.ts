...
const TEST_UPLOAD_IMAGE_PATH = path.join(
...
test("admin login: correct password redirects to dashboard", async ({
  page,
}) => {
...
test("admin menu: displays categories and items", async ({ page }) => {
  ...
  expect(catCount, "should have at least 2 categories").toBeGreaterThanOrEqual(
    2,
  );
...
test("admin menu: item rows have action buttons", async ({ page }) => {
  ...
  await expect(
    firstRow.locator(".btn-sm", { hasText: /Enable|Disable/ }),
  ).toBeVisible();
  await expect(firstRow.locator(".btn-sm", { hasText: "Edit" })).toBeVisible();
  await expect(
    firstRow.locator(".btn-sm.danger", { hasText: "Delete" }),
  ).toBeVisible();
});
...
test("admin menu: every item can change image via quick picker", async ({
  page,
}) => {
  await adminLogin(page);
  ...
  for (let i = 0; i < totalRows; i++) {
    ...
    const thumbBtn = row.locator(".thumb-btn").first();
    await expect(thumbBtn).toBeVisible();
    await thumbBtn.scrollIntoViewIfNeeded();
    await thumbBtn.click();

    const popover = page.locator(".image-swap-popover");
    await expect(popover).toBeVisible();

    const thumb = row.locator(".item-thumb").first();
    const previousSrc = await thumb.getAttribute("src");

    const alternativeOption = popover
      .locator(".image-option:not(.selected)")
      .first();
    await expect(alternativeOption).toBeVisible();
    await alternativeOption.click();

    await expect(
      page.locator('[data-testid="admin-save-feedback"]'),
    ).toContainText("Image saved and synced live.", { timeout: 20000 });

    await expect
      .poll(async () => await thumb.getAttribute("src"), { timeout: 20000 })
      .not.toBe(previousSrc);
  }
});

test("admin menu: uploaded image is saved and shown on live display", async ({
  page,
  context,
}) => {
  await adminLogin(page);
  await page.goto("/admin/menu");
  await page.waitForSelector(".item-row .item-thumb", { timeout: 10000 });

  const targetRow = page
    .locator(".item-row")
    .filter({ has: page.locator(".item-thumb") })
    .first();
  await expect(targetRow).toBeVisible();

  const itemNameRaw = (
    await targetRow.locator(".item-name").first().innerText()
  ).trim();
  const itemName = itemNameRaw.split("\n")[0]?.trim() || itemNameRaw;
  const thumb = targetRow.locator(".item-thumb");

  const livePage = await context.newPage();
  await livePage.goto("/");
  await livePage.waitForSelector('[data-testid="menu-item"]', {
    timeout: 10000,
  });
  const liveItem = livePage
    .locator('[data-testid="menu-item"]', { hasText: itemName })
    .first();
  await expect(liveItem).toBeVisible();

  const liveImage = liveItem.locator("img").first();
  await expect(liveImage).toBeVisible();

  // Force a known baseline image from static library.
  await targetRow.locator(".thumb-btn").click();
  await expect(page.locator(".image-swap-popover")).toBeVisible();
  await page.getByRole("tab", { name: "Library" }).click();
  await page.locator(".image-swap-popover .image-option").first().click();
  await expect(
    page.locator('[data-testid="admin-save-feedback"]'),
  ).toContainText("Image saved and synced live.", { timeout: 20000 });

  await expect
    .poll(async () => await thumb.getAttribute("src"), { timeout: 15000 })
    .toContain("/images/menu/");
  await expect
    .poll(async () => await liveImage.getAttribute("src"), { timeout: 20000 })
    .toContain("/images/menu/");

  // Upload via user interaction and verify sync.
  await targetRow.locator(".thumb-btn").click();
  await expect(page.locator(".image-swap-popover")).toBeVisible();
  await page.getByRole("tab", { name: "Upload" }).click();
  await page
    .locator('[data-testid="image-upload-input"]')
    .setInputFiles(TEST_UPLOAD_IMAGE_PATH);
  await expect(
    page.locator('[data-testid="admin-save-feedback"]'),
  ).toContainText("Image saved and synced live.", { timeout: 20000 });
...
test("admin preview: shows 3 TV panels", async ({ page }) => {
  ...
  // Check panel headers
  await expect(page.locator(".panel-header h3").nth(0)).toContainText(
    "Dumplings",
  );
  await expect(page.locator(".panel-header h3").nth(1)).toContainText(
    "Noodles",
  );
...
test("admin preview: theme toggle switches iframe sources", async ({
  page,
}) => {
  await adminLogin(page);
  await page.click(".nav-item >> text=Live Preview");
...
test("admin theme: has presets including fulala-valentine", async ({
  page,
}) => {
  await adminLogin(page);
  await page.goto("/admin/theme");
  ...
  await expect(
    page.locator(".preset-btn", { hasText: "Fulala-valentine" }),
  ).toBeVisible();
  await expect(
    page.locator(".preset-btn", { hasText: "Fulala" }).first(),
  ).toBeVisible();
});
...
test("admin events: shows 3 event tabs", async ({ page }) => {
  ...
  await expect(
    page.getByRole("button", { name: "Event Packages" }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Catering Menus" }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "School Meals" }),
  ).toBeVisible();
});

// --- 9. Print ---

test("admin print: shows printable menu with all categories", async ({
  page,
}) => {
  await adminLogin(page);
