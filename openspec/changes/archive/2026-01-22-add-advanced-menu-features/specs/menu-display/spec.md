# Spec: Menu Display Capabilities
...
**Related Specs:**
- `customer-ordering` - Modifiers must be selectable during order creation
- `currency` - Prices must respect active currency settings in all layouts
- `event-management` - Event packages may reference menu items displayed in layouts

**Dependencies:**
- Images require `menuItems.imageUrl` field (added in `add-menu-item-images` change)
- Modifiers require schema enums for validation
- Dietary tags require validated enum values
- Layouts require Convex reactivity for real-time updates
