# Spec: Customer Ordering Capabilities
...
**Related Specs:**
- `menu-display` - Menu items with modifiers and dietary tags are displayed and orderable
- `currency` - Order prices must respect active currency settings
- `event-management` - Future integration with event package ordering

**Dependencies:**
- Requires `customerOrders` table with sessionId index
- Requires `menuItems` table with modifiers and dietary tags
- Requires Convex reactivity for real-time updates
- Requires Svelte store for cart state management
- Requires localStorage for sessionId persistence
