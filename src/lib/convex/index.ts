...
export function getConvexClient(): ConvexClient {
  if (!client) {
    if (!CONVEX_URL) {
      throw new Error(
        "Convex URL is not configured. Set VITE_CONVEX_URL or PUBLIC_CONVEX_URL.",
      );
    }
...
export function getConvexClientFromContext(): ConvexClient {
  ...
  if (!client) {
    throw new Error(
      "Convex client not found in context. Did you wrap your app in ConvexClientProvider?",
    );
  }
  return client;
}
...
export function useMutation<Mutation extends FunctionReference<"mutation">>(
  ...
  return async (...args: OptionalRestArgs<Mutation>) => {
    return client.mutation(mutation, args[0] ?? {}) as Promise<
...
export function useAction<Action extends FunctionReference<"action">>(
  ...
  return async (...args: OptionalRestArgs<Action>) => {
    return client.action(action, args[0] ?? {}) as Promise<
